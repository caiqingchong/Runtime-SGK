...
- (void)setImagePathsGroup:(NSArray *)imagePathsGroup
{
    if (imagePathsGroup.count < _imagePathsGroup.count) {
        [self.mainView setContentOffset:CGPointZero animated:NO];
    }
...
- (void)setupTimer
{
    ...
    _timer = timer;
    //NSRunLoopCommonModes 防止滑动过程中出现的线程阻塞问题
...
- (int)currentIndex
{
    ...
    if (_flowLayout.scrollDirection == UICollectionViewScrollDirectionHorizontal) {
        index = (self.mainView.contentOffset.x + _flowLayout.itemSize.width * 0.5) / _flowLayout.itemSize.width;
    } else {
        index = (self.mainView.contentOffset.y + _flowLayout.itemSize.height * 0.5) / _flowLayout.itemSize.height;
    }
    return index;
}
...
- (void)layoutSubviews
{
    ...
    if (self.mainView.contentOffset.x == 0 &&  _totalItemsCount) {
        int targetIndex = 0;
        if (self.infiniteLoop) {
            targetIndex = _totalItemsCount * 0.5;
        }else{
